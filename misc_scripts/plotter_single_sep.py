...
def plot_fitness_diversity(behavior, theory_fitness=None):
    ...
    fitnesses = np.loadtxt('/Users/devendra/workspace/SwarmAggregationGA/output/sep_best_t12_working.csv', ndmin = 2) #, max_rows=300)
    avg_fitnesses = np.loadtxt('/Users/devendra/workspace/SwarmAggregationGA/output/sep_avg_t12_working.csv', ndmin = 2) #, max_rows=300)
    diversity = np.loadtxt('/Users/devendra/workspace/SwarmAggregationGA/output/sep_div_t12_working.csv', ndmin = 2) #, max_rows=300)
    ...
    # Set up colors.
    fitness_color, diverse_color, avg_color = batlow(0.2), batlow(0.6), batlow(0.8)
    ...
    if theory_fitness is not None:
        fitness_ax.plot(x, np.repeat(theory_fitness, len(x)),
                        ...
                        label=r'Cannon et al. (2019), $\lambda = 4$, $\gamma = 6$') #
    ...
    # fitness_ax.plot(x, np.mean(avg_fitnesses, axis=1), color=avg_color,
    #                 label='Avg. Fitness')
    fitness_ax.plot(x, fitnesses, color=fitness_color,
                    label='Best Fitness')
    fitness_ax.plot(x, avg_fitnesses, color=avg_color,
                    label='Avg. Fitness')
    ...
    # Save the figure.
    fig.savefig(osp.join('output', behavior + '_small_250_trial.png'))

plot_fitness_diversity('Sep', 0.84)