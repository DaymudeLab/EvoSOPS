...
pub struct SOPSCoatEnvironmentCMA {
    grid: Vec<Vec<u8>>,
    participants: Vec<Particle>,
    phenotype: [[[f64; 10]; 6]; 10],
    sim_duration: u64,
    fitness_val: f32,
    size: usize,
    arena_layers: u16,
...
impl SOPSCoatEnvironmentCMA {
     ...
     */
     ...
     fn get_obj_dist(&mut self, start: (usize, usize)) -> u16 {
        ...
        while let Some(curr_loc) = buffer.pop_front() {
            for idx in 0..6 {
                let new_i = (curr_loc[0] as i32 + SOPSCoatEnvironmentCMA::directions()[idx].0) as usize;
                let new_j = (curr_loc[1] as i32 + SOPSCoatEnvironmentCMA::directions()[idx].1) as usize;
                let depth = curr_loc[2];
                if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) {
                    match visited.get(&[new_i, new_j]) {
                        Some(_exists) => {}
                        None => {
                            if self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::OBJECT {
                                return (depth+1) as u16;
                            }
                            else if self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::EMPTY || self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::PARTICLE {
    ...
    //  /*
    pub fn init_sops_env(genome: &[[[f64; 10]; 6]; 10], arena_layers: u16, object_layers: u16, coat_layers: u16, seed: u64, granularity: u8, w1: f32, w2: f32, grid_distances: Option<HashMap<[usize; 2], u16>>) -> Self {
        let grid_size = (arena_layers*2 + 1) as usize;
        let mut grid = vec![vec![0; grid_size]; grid_size];
        let mut participants: Vec<Particle> = vec![];
        ...
        let mut grid_rng = SOPSCoatEnvironmentCMA::seed_rng(seed);

        //init grid bounds
        for i in 0..arena_layers {
            let mut j = 1;
            while i+arena_layers+j < (grid_size as u16) {
                grid[i as usize][(i+arena_layers+j) as usize] = SOPSCoatEnvironmentCMA::BOUNDARY;
                grid[(i+arena_layers+j) as usize][i as usize] = SOPSCoatEnvironmentCMA::BOUNDARY;
                j +=1;
            }
        }
        ...
        loop {
            ...
            if (y+total_coat_size) < grid_size && (x+total_coat_size) < grid_size {
                if grid[x][y] != SOPSCoatEnvironmentCMA::BOUNDARY && grid[x][y+total_coat_size] != SOPSCoatEnvironmentCMA::BOUNDARY && grid[x+total_coat_size][y] != SOPSCoatEnvironmentCMA::BOUNDARY && grid[x+total_coat_size][y+total_coat_size] != SOPSCoatEnvironmentCMA::BOUNDARY {
                    obj_loc[0] = x;
                    obj_loc[1] = y;
                    break;
                }   
        ...
        for i in 0..object_size {
            ...
            while j < (i+(object_layers as usize)+1) && j < object_size {
                grid[(obj_loc[0] + (coat_layers as usize) + i) as usize][(obj_loc[1] + (coat_layers as usize) + j) as usize] = SOPSCoatEnvironmentCMA::OBJECT;
                grid[(obj_loc[0] + (coat_layers as usize) + j) as usize][(obj_loc[1] + (coat_layers as usize) + i) as usize] = SOPSCoatEnvironmentCMA::OBJECT;
                j +=1;
            }
        }
        ...
        while participants.len() < num_particles.into() {
            let i = grid_rng.sample(&SOPSCoatEnvironmentCMA::grid_rng(0,grid_size));
            let j = grid_rng.sample(&SOPSCoatEnvironmentCMA::grid_rng(0,grid_size));
            if grid[i][j] == 0 {
                participants.push(Particle {
                    x: i as u8,
                    y: j as u8,
                ...
                grid[i][j] = SOPSCoatEnvironmentCMA::PARTICLE;
            }   
        }
    ...
    pub fn save_distance_grid(&mut self) -> HashMap<[usize; 2], u16> {
        ...
        for i in 0..self.grid.len() {
            for j in 0..self.grid[0].len() {
                if self.grid[i][j] == SOPSCoatEnvironmentCMA::EMPTY || self.grid[i][j] == SOPSCoatEnvironmentCMA::PARTICLE {
    ...
    fn move_particle_to(&mut self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        self.grid[particle.x as usize][particle.y as usize] -= SOPSCoatEnvironmentCMA::PARTICLE;
        
        self.grid[new_i][new_j] += SOPSCoatEnvironmentCMA::PARTICLE;
    ...
    fn get_ext_neighbors_cnt(&self, particle_idx: usize, direction: (i32, i32)) -> (u8, u8, u8) {
        ...
        for idx in 0..6 {
            let new_i = (particle.x as i32 + SOPSCoatEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (particle.y as i32 + SOPSCoatEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == move_i) & (new_j == move_j)) {
                ...
                else if self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::PARTICLE {
                    back_cnt += 1;
                }
            }
        }
        ...
        for idx in 0..6 {
            let new_i = (move_i as i32 + SOPSCoatEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (move_j as i32 + SOPSCoatEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == particle.x.into()) & (new_j == particle.y.into())) {
                // print!("{}",idx);
                let mut position_type = SOPSCoatEnvironmentCMA::FRONT;
                match seen_neighbor_cache.get(&[new_i, new_j]) {
                    Some(_exists) => {
                        position_type = SOPSCoatEnvironmentCMA::MID;
                    }
                    None => {},
                }
                if self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::OBJECT {
                    match position_type {
                        SOPSCoatEnvironmentCMA::FRONT => {
                            front_obj_cnt += 1;
                        }
                        SOPSCoatEnvironmentCMA::MID => {
                            mid_obj_cnt += 1;
                            back_obj_cnt -= 1;
                        }
                        _ => todo!()
    ...
    fn particle_move_possible(&self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) {
            if self.grid[new_i][new_j] == SOPSCoatEnvironmentCMA::EMPTY {
                return true;
            }
            else {
                return false;
    ...
    fn move_particles(&mut self, cnt: usize) {
        ...
        let par_idx = SOPSCoatEnvironmentCMA::move_frng().usize(..self.participants.len());
        
        // Choose a direction at random (out of the 6)
        let move_dir = SOPSCoatEnvironmentCMA::directions()
            [SOPSCoatEnvironmentCMA::move_frng().usize(..SOPSCoatEnvironmentCMA::directions().len())];
        ...
        if self.particle_move_possible(par_idx, move_dir) {
            ...
            if SOPSCoatEnvironmentCMA::move_frng().f64() <= move_prb
            {
