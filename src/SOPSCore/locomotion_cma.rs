 ...
 *  */
pub struct SOPSLocoEnvironmentCMA {
    grid: Vec<Vec<u8>>,
    participants: Vec<Particle>,
    light: Vec<(u8,u8)>,
    orientation: u8,
    phenotype: [[[[f64; 4]; 3]; 4]; 3],
    sim_duration: u64,
    fitness_val: f32,
    size: usize,
    max_fitness_c1: f32,
...
impl SOPSLocoEnvironmentCMA {
     ...
     *  */
    pub fn init_sops_env(genome: &[[[[f64; 4]; 3 ]; 4]; 3], arena_layers: u16, particle_layers: u16, seed: u64, granularity: u8, w1: f32, w2: f32) -> Self {
        ...
        let mut grid_rng = SOPSLocoEnvironmentCMA::seed_rng(seed);
        //init grid bounds
        for i in 0..arena_layers {
            let mut j = 1;
            while i+arena_layers+j < (grid_size as u16) {
                grid[i as usize][(i+arena_layers+j) as usize] = SOPSLocoEnvironmentCMA::BOUNDARY;
                grid[(i+arena_layers+j) as usize][i as usize] = SOPSLocoEnvironmentCMA::BOUNDARY;
                j +=1;
            }
        }
        ...
        while participants.len() < num_particles.into() {
            let i = grid_rng.sample(&SOPSLocoEnvironmentCMA::grid_rng(0,grid_size));
            let j = grid_rng.sample(&SOPSLocoEnvironmentCMA::grid_rng(0,grid_size));
            if grid[i][j] == 0 {
                ...
                grid[i][j] = SOPSLocoEnvironmentCMA::PARTICLE;
     ...
     *  */
     fn get_neighbors_cnt(&self, i: u8, j: u8) -> u8 {
        let mut cnt = 0;
        for idx in 0..6 {
            let new_i = (i as i32 + SOPSLocoEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (j as i32 + SOPSLocoEnvironmentCMA::directions()[idx].1) as usize;
     ...
     fn move_particle_to(&mut self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        if self.orientation == 1 {
            ...
            if particle.x + particle.y >= self.arena_layers as u8 {
                if particle.x + particle.y <= (3*self.arena_layers) as u8 {
            ...
            if new_i + new_j >= self.arena_layers as usize {
                if new_i + new_j <= (3*self.arena_layers) as usize {
        ...
        if self.orientation == 1 && sensing_light == 1 {
            if 2*new_i >= new_j {
                ...
                if 2*particle.x - particle.y == (2*new_i - new_j) as u8{
                    ...
                    while y >= 0 {
                        if self.grid[x as usize][y as usize] == SOPSLocoEnvironmentCMA::PARTICLE {
        ...
        else if self.orientation == 1 && future_sensing_light == 1 {
            // moving from no light to light
            // adjust future light value
            if future_sensing_light == 1 {
                self.light[2*new_i - new_j] = (new_i as u8, new_j as u8);
            }
        ...
        else if self.orientation == 2 && future_sensing_light == 1 {
            // moving from no light to light
            // adjust future light value
            if future_sensing_light == 1 {
                self.light[2*new_j - new_i] = (new_i as u8, new_j as u8);
        ...
        else if self.orientation == 3 && future_sensing_light == 1 {
            // moving from no light to light
            // adjust future light value
            if future_sensing_light == 1 {
                self.light[new_i + new_j - self.arena_layers as usize] = (new_i as u8, new_j as u8);
        ...
        // move the particle
        self.grid[particle.x as usize][particle.y as usize] = SOPSLocoEnvironmentCMA::EMPTY;
        self.grid[new_i][new_j] = SOPSLocoEnvironmentCMA::PARTICLE;
     ...
     *  */
     fn get_ext_neighbors_cnt(&self, particle_idx: usize, direction: (i32, i32)) -> (u8, u8, u8, u8) {
        ...
        for idx in 0..6 {
            let new_i = (particle.x as i32 + SOPSLocoEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (particle.y as i32 + SOPSLocoEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == move_i) & (new_j == move_j)) {
                ...
                if self.grid[new_i][new_j] == SOPSLocoEnvironmentCMA::PARTICLE {
                    back_cnt += 1;
                }
            }
        }
        ...
        for idx in 0..6 {
            let new_i = (move_i as i32 + SOPSLocoEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (move_j as i32 + SOPSLocoEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == particle.x.into()) & (new_j == particle.y.into())) {
                let mut position_type = SOPSLocoEnvironmentCMA::FRONT;
                match seen_neighbor_cache.get(&[new_i, new_j]) {
                    Some(_exists) => {
                        position_type = SOPSLocoEnvironmentCMA::MID;
                    }
                    None => {},
                }
                if self.grid[new_i][new_j] == SOPSLocoEnvironmentCMA::PARTICLE {
                    match position_type {
                        SOPSLocoEnvironmentCMA::FRONT => {
                            front_cnt += 1;
                        }
                        SOPSLocoEnvironmentCMA::MID => {
                            mid_cnt += 1;
                            back_cnt -= 1;
                        }
                        _ => todo!()
     ...
     *  */
     fn particle_move_possible(&self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) {
            ...
            if self.grid[new_i][new_j] == SOPSLocoEnvironmentCMA::PARTICLE || self.grid[new_i][new_j] == SOPSLocoEnvironmentCMA::BOUNDARY {
                return false;
            } else {
    ...
    fn move_particles(&mut self, cnt: usize) {
        ...
        let par_idx = SOPSLocoEnvironmentCMA::move_frng().usize(..self.participants.len());
        // Choose a direction at random (out of the 6)
        let move_dir = SOPSLocoEnvironmentCMA::directions()
                [SOPSLocoEnvironmentCMA::move_frng().usize(..SOPSLocoEnvironmentCMA::directions().len())];
        ...
        if self.particle_move_possible(par_idx, move_dir) {
            ...
            let move_prb = self.phenotype[light_idx as usize][back_cnt as usize][mid_cnt as usize][front_cnt as usize];
            if SOPSLocoEnvironmentCMA::move_frng().f64() <= move_prb
            {
