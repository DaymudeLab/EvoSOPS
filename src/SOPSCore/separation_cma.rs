 ...
 *  */
pub struct SOPSepEnvironmentCMA {
    grid: Vec<Vec<u8>>,
...
impl SOPSepEnvironmentCMA {
     ...
     *  */
    pub fn init_sops_env(genome: &[[[f64; 10]; 6]; 10], arena_layers: u16, particle_layers: u16, seed: u64, granularity: u8, w1: f32, w2: f32) -> Self {
        ...
        let mut grid_rng = SOPSepEnvironmentCMA::seed_rng(seed);
        //init grid bounds
        for i in 0..arena_layers {
            let mut j = 1;
            while i+arena_layers+j < (grid_size as u16) {
                grid[i as usize][(i+arena_layers+j) as usize] = SOPSepEnvironmentCMA::BOUNDARY;
                grid[(i+arena_layers+j) as usize][i as usize] = SOPSepEnvironmentCMA::BOUNDARY;
                j +=1;
            }
        }
        ...
        while participants.len() < num_particles.into() {
            ...
            let i = grid_rng.sample(&SOPSepEnvironmentCMA::grid_rng(0,grid_size));
            let j = grid_rng.sample(&SOPSepEnvironmentCMA::grid_rng(0,grid_size));
            if grid[i][j] == 0 {
    ...
    fn get_neighbors_cnt(&self, i: u8, j: u8, n_clr: u8) -> (u8, u8, u8) {
        ...
        let mut n_clr_cnt = 0;
        for idx in 0..6 {
            let new_i = (i as i32 + SOPSepEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (j as i32 + SOPSepEnvironmentCMA::directions()[idx].1) as usize;
    ...
    fn move_particle_to(&mut self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) {
            ...
            if self.grid[new_i][new_j] == SOPSepEnvironmentCMA::BOUNDARY || self.grid[new_i][new_j] == self.grid[particle.x as usize][particle.y as usize] {
                return false;
            } else if self.grid[new_i][new_j] == SOPSepEnvironmentCMA::EMPTY {
                // simple move in empty location
                // println!("Particle at {},{},c{} moves to {},{}", particle.x, particle.y, particle.color, new_i, new_j);
                self.grid[particle.x as usize][particle.y as usize] = SOPSepEnvironmentCMA::EMPTY;
     ...
     *  */
     fn check_connectivity(&self, particle_idx: usize, direction: (i32, i32)) -> bool {
        ...
        for idx in 0..6 {
            let new_i = (particle.x as i32 + SOPSepEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (particle.y as i32 + SOPSepEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == move_i) & (new_j == move_j)) {
                ...
                if self.grid[new_i][new_j] != SOPSepEnvironmentCMA::EMPTY && self.grid[new_i][new_j] != SOPSepEnvironmentCMA::BOUNDARY  {
                    back_cnt += 1;
        ...
        for idx in 0..6 {
            let new_i = (move_i as i32 + SOPSepEnvironmentCMA::directions()[idx].0) as usize;
            let new_j = (move_j as i32 + SOPSepEnvironmentCMA::directions()[idx].1) as usize;
            if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) & !((new_i == particle.x.into()) & (new_j == particle.y.into())) {
                // print!("{}",idx);
                let mut position_type = SOPSepEnvironmentCMA::FRONT;
                match seen_neighbor_cache.get(&[new_i, new_j]) {
                    Some(_exists) => {
                        position_type = SOPSepEnvironmentCMA::MID;
                    }
                    None => {
                    },
                }
        ...
        if mid_cnt > 0 {
            ...
            // check if back and front are connected to mid
                ...
                for loc in back_neighbor_cache.keys() {
                    ...
                    for idx in 0..6 {
                        let new_i = (loc[0] as i32 + SOPSepEnvironmentCMA::directions()[idx].0) as usize;
                        let new_j = (loc[1] as i32 + SOPSepEnvironmentCMA::directions()[idx].1) as usize;
                        if (0..self.grid.len()).contains(&new_i) && (0..self.grid.len()).contains(&new_j) && !((new_i == particle.x.into()) && (new_j == particle.y.into())) {
                            // print!("{}",idx);
     ...
     *  */
     fn particle_move_possible(&self, particle_idx: usize, direction: (i32, i32)) -> u8 {
        ...
        if (0..self.grid.len()).contains(&new_i) & (0..self.grid.len()).contains(&new_j) {
            ...
            if self.grid[new_i][new_j] == SOPSepEnvironmentCMA::BOUNDARY {
                return 0;
            } else {
                // can move the particle
                if self.grid[new_i][new_j] == SOPSepEnvironmentCMA::EMPTY {
                    return 2;
                }
    ...
    fn move_particles(&mut self, cnt: usize) {
        ...
        let par_idx = SOPSepEnvironmentCMA::move_frng().usize(..self.participants.len());
        // Choose a direction at random (out of the 6)
        let move_dir = SOPSepEnvironmentCMA::directions()
                        [SOPSepEnvironmentCMA::move_frng().usize(..SOPSepEnvironmentCMA::directions().len())];
        ...
        match self.particle_move_possible(par_idx, move_dir) {
            1 => {
                ...
                if SOPSepEnvironmentCMA::move_frng().f64() <= move_prb
                {
