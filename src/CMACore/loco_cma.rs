...
pub struct LocoCMA {
    ...
    mut_rate: f64,
    granularity: u8,
    genome_cache: HashMap<[[[[OrderedFloat<f64>; 4]; 3]; 4]; 3], f64>,
    perform_cross: bool,
    sizes: Vec<(u16,u16)>,
    trial_seeds: Vec<u64>,
...
impl LocoCMA {
    ...
    fn cross_pnt() -> Uniform<u16> {
        Uniform::new_inclusive(0, LocoCMA::GENOME_LEN-1)
    }
    ...
    pub fn init_ga(
        ...
        println!("Thresholds: UPPER: {}, LOWER {}", LocoCMA::UPPER_T, LocoCMA::LOWER_T);
        let mut starting_pop: Vec<LocoGenome> = vec![];

        for _ in 0..population_size {
            //init genome
            let mut genome: [[[[f64; 4]; 3]; 4]; 3] = [[[[0_f64; 4]; 3]; 4]; 3];
            ...
            for n in 0_u8..3 {
                for i in 0_u8..4 {
                    for j in 0_u8..3 {
                        for k in 0_u8..4 {
                            genome[n as usize][i as usize][j as usize][k as usize] = LocoCMA::rng().sample(LocoCMA::genome_prob_init_rng())
                        }
                    }
                }
            }
        ...
        let genome_cache: HashMap<[[[[OrderedFloat<f64>; 4]; 3]; 4]; 3], f64> = HashMap::new();

        // Initial CMA-ES values
        let mut mean = DMatrix::from_element(Self::GENOME_LEN.into(), 1, 0.5);
        for i in 0..Self::GENOME_LEN.into() {
            mean[i] = LocoCMA::rng().sample(LocoCMA::mean_init_rng(search_interval[0].0, search_interval[0].1)) as f64
        }
        ...
        let covariance_matrix = DMatrix::from_diagonal_element(LocoCMA::GENOME_LEN.into(), LocoCMA::GENOME_LEN.into(), 1.0);
        ...
        LocoCMA {
            ...
            max_div: ((granularity-1) as u32)*(LocoCMA::GENOME_LEN as u32),
            w1,
            w2,
            random_seed,
            mean,
    ...
    fn genome_to_column_vector(&self, genome: [[[[f64; 4]; 3]; 4]; 3]) ->  Matrix<f64, Dyn, Dyn, VecStorage<f64, Dyn, Dyn>> {
        let mut x: Vec<f64> = vec![];
        for n in 0_u8..3 {
            for i in 0_u8..4 {
                for j in 0_u8..3 {
                    for k in 0_u8..4 {
                        x.push(genome[n as usize][i as usize][j as usize][k as usize]);
                    }
                }
            }
        }
        DMatrix::from_row_iterator(LocoCMA::GENOME_LEN.into(), 1, x.into_iter())
    }

    // Takes a column vector and returns an equivalent genome
    fn column_vector_to_genome(&self, column: Matrix<f64, Dyn, Dyn, VecStorage<f64, Dyn, Dyn>>) -> [[[[f64; 4]; 3]; 4]; 3] {
        let mut genome: [[[[f64; 4]; 3]; 4]; 3] = [[[[0_f64; 4]; 3]; 4]; 3];
    ...
    fn sample_new_population(&mut self) {
                    ...
                    for i in 0..4 {
                        for j in 0..3 {
                            for k in 0..4 {
                                ...
                                buff.extend(genome.string[n][i][j][k].to_be_bytes());
                            }
                        }
                    }
                }
        ...
        for _ in 0..self.population.len() as usize{
            ...
            for i in 0..Self::GENOME_LEN as usize{
                z_k[(i, 0)] = LocoCMA::rng().sample(&LocoCMA::normal0_1());
            }
    ...
    fn step_through(&mut self, gen: u16) -> f32 {
        ...
        self.population
            ...
            .for_each(|(idx, genome)| {
                ...
                let mut genome_s: [[[[OrderedFloat<f64>; 4]; 3]; 4]; 3] = [[[[OrderedFloat(0_f64); 4]; 3]; 4]; 3];
                for n in 0..3 {
                    for i in 0..4 {
                        for j in 0..3 {
                            for k in 0..4 {
                                genome_s[n][i][j][k] = OrderedFloat(genome.string[n][i][j][k]);
                            }
                        }
                    }
                }
        ...
        self.population.par_iter_mut().for_each(|genome| {
            ...
            let mut genome_s = genome.string.clone();
            ...
            let fitness_tot: f64 = trials_vec.clone()
                .into_par_iter()
                .map(|trial| {
                    let mut genome_env = SOPSLocoEnvironmentCMA::init_sops_env(&genome_s, trial.0.0, trial.0.1, trial.1.into(), granularity, w1, w2);
        ...
        for idx in 0..self.population.len() {
            ...
            for n in 0..3 {
                for i in 0..4 {
                    for j in 0..3 {
                        for k in 0..4 {
                            genome_s[n][i][j][k] = OrderedFloat(self.population[idx].string[n][i][j][k]);
                        }
                    }
                }
            }
                    ...
                    for p in 0..4 {
                        for q in 0..3 {
                            for k in 0..4 {
                                ...
                                //dis_sum += dis as u16;
                                let dis = (genome1.string[n][p][q][k] - genome2.string[n][p][q][k]).abs();
                                dis_sum += dis as f32;
                            }
                        }
                    }
    ...
    pub fn run_through(&mut self) {
        let mut diversity_q: VecDeque<f32> = VecDeque::with_capacity(LocoCMA::BUFFER_LEN);
        // Run the GA for given #. of Generations
        for gen in 0..self.max_gen {
            println!("Starting Gen:{}", gen);
            let now = Instant::now();
            if diversity_q.len() == LocoCMA::BUFFER_LEN { diversity_q.pop_front(); }
            ...
            match self.div_state {
                DiversThresh::INIT => {
                    if norm_avg_div <= LocoCMA::LOWER_T {
                        self.increase_mut();
                        self.div_state = DiversThresh::LOWER_HIT;
                    }
                },
                DiversThresh::LOWER_HIT => {
                    if norm_avg_div >= LocoCMA::UPPER_T {
                        self.lower_mut();
