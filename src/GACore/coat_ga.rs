...
impl CoatGA {
    ...
    fn step_through(&mut self, gen: u16) -> f32 {
        ...
        let trials_vec: Vec<((u16,u16, u16),u64)> = self
            .sizes.clone()
            .into_iter()
            .zip(seeds)
        ...
        self.population.par_iter_mut().for_each(|genome| {
            ...
            let fitness_tot: f64 = trials_vec.clone()
                ...
                .map(|trial| {
                    // let now = Instant::now();
                    let mut genome_env = SOPSCoatEnvironment::init_sops_env(&genome_s, trial.0.0, trial.0.1, trial.0.2, trial.1.into(), granularity, w1, w2);
