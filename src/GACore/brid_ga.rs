...
pub struct BridGA {
    ...
    mut_rate: f64,
    granularity: u8,
    genome_cache: HashMap<[[[[[u8; 3]; 2]; 4]; 3]; 4], f64>,
    arena_parameters: Vec<(u16, u16, u16, u16)>,
    weights: (f32, f32, f32, f32)   
}

impl BridGA {
    const GENOME_LEN: u16 = 3 * 3 * 4 * 2 * 4;
    #[inline]
    ...
    pub fn init_ga(
        ...
        for _ in 0..population_size {
            let mut genome: [[[[[u8; 3]; 2]; 4]; 3]; 4] = [[[[[0_u8; 3]; 2]; 4]; 3]; 4];
        ...
        let genome_cache: HashMap<[[[[[u8; 3]; 2]; 4]; 3]; 4], f64> = HashMap::new();
    ...
    // mutate genome based on set mutation rate for every gene of the genome
    fn mutate_genome(&self, genome: &[[[[[u8; 3]; 2]; 4]; 3]; 4]) -> [[[[[u8; 3]; 2]; 4]; 3]; 4] {
     ...
     *  */
     fn generate_offspring(&self, parent1: &[[[[[u8; 3]; 2]; 4]; 3]; 4], parent2: &[[[[[u8; 3]; 2]; 4]; 3]; 4]) -> [[[[[u8; 3]; 2]; 4]; 3]; 4] {
        let mut new_genome: [[[[[u8; 3]; 2]; 4]; 3]; 4] = [[[[[0_u8; 3]; 2]; 4]; 3]; 4];
     ...
     *  */
     fn generate_new_pop(&mut self) {
        ...
        let mut selected_g: Vec<[[[[[u8; 3]; 2]; 4]; 3]; 4]> = vec![];
        let mut crossed_g: Vec<[[[[[u8; 3]; 2]; 4]; 3]; 4]> = vec![];
